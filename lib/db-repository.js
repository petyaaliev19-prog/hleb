...
function parseOrderNumber(orderId) {
  const numeric = Number.parseInt(String(orderId).replace(/\D/g, ""), 10);
  ...
  function createOrder(payload) {
    ...
    const validatedItems = items.map((item) => {
      ...
      if (seenProductIds.has(item.productId)) {
        throw new Error("Duplicate order items are not allowed");
      }
    ...
    const orderIds = db.prepare("SELECT id FROM orders").all();
    const nextNumeric = Math.max(1044, ...orderIds.map((order) => parseOrderNumber(order.id))) + 1;
